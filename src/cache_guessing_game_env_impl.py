...
class CacheGuessingGameEnv(gym.Env):
	...
	Actions:
		action is one-hot encoding
  		v = | attacker_addr | ( flush_attacker_addr ) | v | victim_guess_addr | ( guess victim not access ) | (random victim access) |
	...
	def __init__(self, env_config={
	 ...
	 "cache_configs": {
		...
		self.attacker_address_space = range(self.attacker_address_min, self.attacker_address_max + 1)  # start with one attacker cache line
	...
	def step(self, action):
		...
		# unpack the action to adapt to slightly different RL framework
		...
		is_victim_random = action[5]									  # check wheter to randomize victim address when invoked	
		...
		if self.step_count >= self.window_size - 1:
			...
			if is_victim == True: #or is_victim_random == True:
				if self.allow_victim_multi_access == True or self.victim_accessed == False:
					...
					if True: #self.configs['cache_1']["rep_policy"] == "plru_pl": no need to distinuish pl and normal rep_policy
						if self.victim_address <= self.victim_address_max:
							...
							domain_id = identity['victim'] # indicates victim has accessed the cache
       
						elif is_victim_random == True:
				...
				if is_guess == True:
					...
					# 	1. normal scenario
					# 	2. empty victim access scenario: victim_addr parsed is victim_addr_e, and self.victim_address is also victim_addr_e + 1
		...
		# the observation (r.time) in this case must be consistent with the observation space
		# return observation, reward, done?, info
    	...
    	# the observation should not obverve the victim latency. thus, we put victim latency in the info
	...
	def set_victim(self, victim_address=-1):
  	...
  	# fake reset the environment, just set a new victim addr. the actual physical state of the cache does not change
	def _reset(self, victim_address=-1):
  	...
  	# use to render the result, not implemented
	...
	def close(self):
  	...
  	# use a given seed to randomize the cache, so that we can set the same state for randomization
	...
	def get_act_space_dim(self):
  	...
  	# same as print() when self.verbose == 1, otherwise does not do anything  	
	...
	address, is_guess, is_victim, is_flush, victim_addr, is_victim_random
	'''
	def parse_action(self, action): # for one-hot encoding, inital values are all set to 0. when each action is activated then its value is 1
		address = 0
		is_guess = 0
		is_victim = 0
		is_flush = 0
		victim_addr = 0 
		is_victim_random = 0 # additional action to mark random victim access when invoked
		...
		if self.flush_inst == False:
			if action < len(self.attacker_address_space):
				...
				is_victim = 1
			elif action < len(self.attacker_address_space) + 1 + len(self.victim_address_space):
				is_guess = 1
				victim_addr = action - ( len(self.attacker_address_space) + 1 ) 
			else: # if action = len(self.attacker_address_space) + 1 + len(self.victim_address_space)
   				is_victim_random = 1
		...
		return [ address, is_guess, is_victim, is_flush, victim_addr, is_victim_random ] # append 'random victim access' to the end

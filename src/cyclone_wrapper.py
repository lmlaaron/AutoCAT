...
#class CCHunterWrapper(gym.Env):
    def __init__(self,
        ...
        self.cyclone_bucket_size = self.env_config.cache_configs.cache_1.blocks / self.env_config.cache_configs.cache_1.associativity / self.cyclone_num_buckets

        self.cyclone_buckets = [[0]* self.cyclone_num_buckets ] * self.cyclone_window_size
    ...
    def step(self, action):
        ...
        if "cyclic_set_index" in info and info["cyclic_set_index"] != -1: 
            set = info["cyclic_set_index"]
            cyclone_counter[set / cyclone_bucket_size ][self.step_count ] += 1

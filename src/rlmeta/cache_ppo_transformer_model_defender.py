...
class CachePPOTransformerModelDefender(PPOModel):
    def __init__(self,
        ...
        self.w_embed = nn.Embedding(2, 2)
        self.s_embed = nn.Embedding(4, 4)
        self.lock_embed = nn.Embedding(self.output_dim, 8)
        # self.lock_embed = nn.Embedding(256, 8)
        ...
        print("**************************in cache ppo transformer model defender  ", self.output_dim)
        self._device = None
    ...
    def forward(self, obs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        l, v, lock, stp, w, s = torch.unbind(obs, dim=-1)
        ...
        lock = self.make_embedding(lock, self.lock_embed)

        x = torch.cat((l, v, lock, stp, w, s), dim=-1)
        x = self.linear_i(x)

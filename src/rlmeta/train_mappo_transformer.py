...
@hydra.main(config_path="./config", config_name="mappo_exp")
...
def main(cfg):
    ...
    t_env_fac = CacheEnvWrapperFactory(cfg.env_config, training=True)
    e_env_fac = CacheEnvWrapperFactory(cfg.env_config, training=False)
    env = t_env_fac(0)
    cfg.model_config["cache_state_size"] = cfg.env_config["cache_configs"][
        "cache_1"]["blocks"]
    cfg.model_config["output_dim"] = env.action_space.n

    train_model = CacheMAPPOTransformerModel(**cfg.model_config).to(
        cfg.train_device)

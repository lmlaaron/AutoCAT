...
class MACovertCallbacks(EpisodeCallbacks):
    def __init__(self):
        super().__init__()
    ...
    def on_episode_step(self, index: int, step: int, action: Action,
                        timestep: TimeStep) -> None:
        attacker_info = timestep['receiver'].info
        if attacker_info["is_guess"] and attacker_info['action_mask']['receiver']:
            self.tot_guess += 1
            self.acc_guess += int(attacker_info["guess_correct"])
        detector_info = timestep['sender'].info
        #self.tot_detect += 1
        #self.acc_detect += int(detector_info["guess_correct"])
        if timestep['sender'].done:
            self.tot_detect = 1
            self.acc_detect += int(detector_info["guess_correct"])
            self._custom_metrics["detector_correct_rate"] = self.acc_detect / self.tot_detect
            if attacker_info['action_mask']['receiver']:
                if self.tot_guess>0:

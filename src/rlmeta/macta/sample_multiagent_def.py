...
from env import CacheAttackerDefenderEnvFactory
...
def run_loop(env: Env, agents: PPOAgent, victim_addr=-1) -> Dict[str, float]:
    episode_length = 0
    atk_episode_reward = 0.0
    defender_count = 0.0  
    defender_acc = 0.0  # detector_acc = 0.0
    num_total_guess = 0.0
    num_total_correct_guess = 0.0
    num_cache_miss = 0
    num_total_cache_miss = 0
    atk_success_rate = 0.0
    ...
    while not timestep["__all__"].done:
        ...
        #    actions['defender'] = Action(0, actions['defender'].info)
        ...
        episode_length += 1
        atk_episode_reward += timestep['attacker'].reward
        ...
        num_cache_miss = timestep["defender"].info.get("latency", 0)
        if num_cache_miss == 1:
            num_total_cache_miss += num_cache_miss

        try:
            defender_action = actions['defender'].action.item()
        except:
            defender_action = actions['defender'].action
        if timestep["__all__"].done and defender_action is not None: #==1:
            defender_count += 1
    ...
    metrics = {
        ...
        "atk_episode_reward": atk_episode_reward,
...
def tournament(env,
    ...
    defender_list = cfg.defenders
    result = {}
    with open('tournament.txt','w') as f:
        for defender in defender_list:
            for attacker in attacker_list:
                #if defender[1].split('/')[-3:-1] == attacker[1].split('/')[-3:-1]:
                #    print(defender[1].split('/')[-3:-1], attacker[1].split('/')[-3:-1])
                #    print("from same training instance, skipping")
                #    print(defender, attacker)
                #    continue
                
                f.write(defender[0]+defender[1]+'\n')
                f.write(attacker[0]+attacker[1]+'\n')
                print(defender)
                print(attacker)
                result[defender[0]+' '+attacker[0]]={}
    
                # defender
                if "Cyclone" in defender[0]:
                    defender_agent = CycloneAgent(cfg.env_config, svm_model_path=defender[1], mode='active')
                elif "CC-Hunter" in defender[0]:
                    defender_agent = CCHunterAgent(cfg.env_config)
                elif "None" in defender[0]:
                    defender_agent = RandomAgent(1)
                else:
                    cfg.model_config["output_dim"] = 16
                    cfg.model_config["step_dim"] = 66
                    defender_params = torch.load(defender[1], map_location='cuda:1')
                    defender_model = CachePPOTransformerModel(**cfg.model_config)
                    defender_model.load_state_dict(defender_params)
                    defender_model.eval()
                    defender_agent = PPOAgent(defender_model, deterministic_policy=cfg.deterministic_policy)
                ...
                for trace_file in cfg.trace_files:
                    ...
                    agents = {"attacker": attacker_agent, "defender": defender_agent, "benign": benign_agent}
                    ...
                    result[defender[0]+' '+attacker[0]][trace_file]=metrics_mean['def_action_rate']
...
def head2head(env,
    ...
    # defender
    cfg.model_config["output_dim"] = 16  # NOTE: change for larger assoc
    ...
    #print('defender model: ', detector_model)
    #detector_model.load_state_dict(detector_params)
    #detector_model.eval()

    # Create agent
    #attacker_agent = PPOAgent(attacker_model, deterministic_policy=cfg.deterministic_policy)
    attacker_agent = PPOAgent(cfg.env_config)
    #attacker_agent = PrimeProbeAgent(cfg.env_config)
    #attacker_agent = RandomAgent(1)

    #defender_agent = RandomAgent(1)
    #detector_agent = PPOAgent(detector_model, deterministic_policy=cfg.deterministic_policy)
    defender_agent = PPOAgent(cfg.env_config)
...
#@hydra.main(config_path="./config", config_name="sample_multiagent") #"641-641-31-FR")# None)
...
def main(cfg):
    ...
    cfg.model_config["output_dim"] = 16
    cfg.model_config["step_dim"] += 2
    defender_params = torch.load(cfg.defender_checkpoint, map_location='cuda:1')
    defender_model = CachePPOTransformerModel(**cfg.model_config)
    defender_model.load_state_dict(defender_params)
    defender_model.eval()

...
class CachePPOTransformerModelPoolDefender(CachePPOTransformerModelDefender):
    ...
    def act(
        ...
        with torch.no_grad():
            ...
            # threshold = -2
            # new_logpi = F.softmax(logpi, dim=-1)
            # new_logpi = torch.gt(logpi, threshold).float()
            # print("^^^^^^^^^^^in cache ppo transformer model defender  ", new_logpi, "  _______________________", logpi)
            ...
            # batch_size, num_actions = greedy_action.size()
            # sample_batch_size, sample_num_actions = sample_action.size()
            # logpi_batch_size, logpi_num_actions = logpi.size()
            # for t in range(sample_batch_size):
            #     for u in range(sample_num_actions):
            #         sample_action[t][u] = random.randint(0, 255)
            # for i in range(batch_size):
            #     for j in range(num_actions):
            #         new_action = 0
            #         for y in range(logpi_num_actions):
            #             new_action += new_logpi[i][y] * (2**y)
            #         greedy_action[i][j] = new_action  #replace 100 with the decimal number of the locked bits
            # action = torch.where(d, greedy_action, sample_action)

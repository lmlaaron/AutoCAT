...
class AttackerCacheGuessingGameEnv(gym.Env):
    ...
    def __init__(self, env_config={
        ...
        "cache_configs": {
            # YAML config file for cache simulation
            "architecture": {
                "word_size": 1,  # bytes
                "block_size": 1,  # bytes
                "write_back": True
        ...
        # prefetcher: "none" "nextline" "stream" https://my.eng.utah.edu/~cs7810/pres/14-7810-13-pref.pdf
        ...
        self.atk_length_violation_reward = env_config[
            "atk_length_violation_reward"] if "atk_length_violation_reward" in env_config else -10000
        self.atk_victim_access_reward = env_config[
            "atk_victim_access_reward"] if "atk_victim_access_reward" in env_config else -10
        self.atk_victim_miss_reward = env_config[
            "atk_victim_miss_reward"] if "atk_victim_miss_reward" in env_config else -10000 \
            if self.force_victim_hit else self.atk_victim_access_reward
        self.atk_double_victim_access_reward = env_config[
            "atk_double_victim_access_reward"] if "atk_double_victim_access_reward" in env_config else -10000
        self.allow_victim_multi_access = env_config[
            "allow_victim_multi_access"] if "allow_victim_multi_access" in env_config else True
        ...
        if self.rerandomize_victim:
            # perm = permutations(list(range(min(self.victim_address_min, self.attacker_address_min),
            # max(self.victim_address_max, self.attacker_address_max) + 1 )))
            # self.perm = list(perm)
        ...
        self.max_box_value = max(self.window_size + 2, 2 * len(self.attacker_address_space) + 1 + len(
            self.victim_address_space) + 1)  # max(self.window_size + 2, len(self.attacker_address_space) + 1)
        ...
        if self.configs['cache_1']["rep_policy"] == "plru_pl":  # pl cache victim access always uses locked access
            assert (
                    self.victim_address_min == self.victim_address_max)  # for plru_pl cache, only 1 address allowed
            self.vprint("[init] victim access %d locked cache line" % self.victim_address_max)
            self.l1.read(hex(self.ceaser_mapping(self.victim_address_max))[2:], self.current_step,
                         replacement_policy.PL_LOCK, domain_id='v')
    ...
    def remap(self):
        if not self.rerandomize_victim:
            self.mapping_func = lambda addr: addr
        else:
            self.vprint("doing remapping!")
            random.shuffle(self.perm)

    # ceaser remapping. addr is integer not string
        ...
        if self.step_count >= self.window_size - 1:
            ...
            if is_victim == True or is_victim_random == True:
                ...
                if self.allow_victim_multi_access == True or self.victim_accessed == False:
                    ...
                    if True:
                        if is_victim_random:
                            ...
                            t, _ = self.l1.read(hex(self.ceaser_mapping(victim_random))[2:],
                                                self.current_step)  # , domain_id='v')
                            t = t.time
                            ...
                            t, _ = self.l1.read(hex(self.ceaser_mapping(self.victim_address))[2:],
                                                self.current_step)  # , domain_id='v')
                ...
                if is_guess:
                    ...
                    lat, _ = self.l1.read(hex(self.ceaser_mapping(int('0x' + address, 16)))[2:],
                                          self.current_step)  # , domain_id='a')
    ...
    def reset(self, victim_address=-1,
        ...
        if self.configs['cache_1']["rep_policy"] == "plru_pl":  # pl cache victim access always uses locked access
            assert (
                    self.victim_address_min == self.victim_address_max)  # plru_pl cache: only 1 address allowed
            self.vprint("[reset] victim access %d locked cache line" % self.victim_address_max)
            # lat, cyclic_set_index, cyclic_way_index = self.l1.read(
            # hex(self.ceaser_mapping(self.victim_address_max))[2:], self.current_step, replacement_policy.PL_LOCK,
            # domain_id='v')
    ...
    '''evaluate the correctness of an action sequence (action+ latency) 
  action_buffer: list [(action, latency)]'''
    ...
    def _reset(self, victim_address=-1):
        ...
        self.victim_accessed = False
        if victim_address == -1:
            if not self.allow_empty_victim_access:
                self.victim_address = random.randint(self.victim_address_min, self.victim_address_max)
    ...
    def _randomize_cache(self, mode="none", seed=-1):
        ...
        for _ in range(self.cache_size * 2):
            if mode == "victim":
                ...
                addr = random.randint(self.victim_address_min, self.victim_address_max) \
                    if random.randint(0, 1) == 1 else random.randint(
                    self.attacker_address_min, self.attacker_address_max)
    ...
    def parse_action(self, action):
        ...
        if not self.flush_inst:
            if action < len(self.attacker_address_space):  # if action < 4
                ...
                is_guess = 1
                victim_addr = action - (
                        len(self.attacker_address_space) + 1 + 1)
                # cause 1 assigned to is_victim_random is at atk addr space+1
                # victim_addr = 6 - 4 + 2 = 4 or 7 - 4 + 2 = 5
        else:

...
class AttackerCacheGuessingGameEnv(gym.Env):
    ...
    def __init__(self, env_config={
        ...
        self.atk_victim_miss_reward = env_config[
            "atk_victim_miss_reward"] if "atk_victim_miss_reward" in env_config else -10000 if self.force_victim_hit else self.atk_victim_access_reward
        ...
        if self.configs['cache_1']["rep_policy"] == "plru_pl":  # pl cache victim access always uses locked access
            assert (
                        self.victim_address_min == self.victim_address_max)  # for plru_pl cache, only one address is allowed
    ...
    def parse_action(self, action):
        ...
        if not self.flush_inst:
            if action < len(self.attacker_address_space):  # if action < 4
                ...
                is_guess = 1
                victim_addr = action - (
                            len(self.attacker_address_space) + 1 + 1)  # because the one that assigned to the is_victim_random is at the attacker address space+1

...
class CacheSimulatorP1Wrapper(gym.Env):
    def __init__(self, env_config):
        ...
        self.observation_space = spaces.Box(low=-1, high=self.max_box_value, shape=(self.window_size, self.feature_size *  self.copy))
        
        # merge all guessing into one action
        self.action_space_size = self.env.action_space.n#(self.env.action_space.n - self.secret_size+1)
        print(self.env.action_space.n)
        print(self.env.get_act_space_dim())
        self.action_space = self.env.action_space #spaces.Discrete(self.action_space_size)
        
        # instantiate the environment
        self.env_list.append(CacheGuessingGameEnv(env_config))
        self.env_config['verbose'] = False
        for _ in range(1, self.copy):
            self.env_list.append(CacheGuessingGameEnv(env_config))
        ...
        if self.env_list[0].allow_empty_victim_access == False:
            self.secret = random.randint(self.victim_address_min, self.victim_address_max)
        else:
            self.secret = random.randint(self.victim_address_min, self.victim_address_max+1)

        for i in range(self.copy):#range(self.victim_address_min, self.victim_address_max+1):
            self.victim_addr_arr.append(self.secret)
    ...
    def step(self, action):
        ...
        if False: #action == self.action_space_size - 1: # guessing action
